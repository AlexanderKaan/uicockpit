...
- Elevation: ${({ flat: 'Flat', soft: 'Subtle', raised: 'Raised', layered: 'Deep' } as const)[cfg.surfaceDepth]} (ramp ${resolveDepth(cfg.surfaceDepth).contrast} · shadow ${resolveDepth(cfg.surfaceDepth).elevation}) · Surface: ${({ outlined: 'Outlined (box + border)', filled: 'Filled (fill, no border)', plain: 'Plain (underline / seamless)' } as const)[cfg.surface]} · Border: ${cfg.borders} · Motion: ${cfg.motion}
