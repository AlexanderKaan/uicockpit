...
- Elevation: ${({ flat: 'Flat', soft: 'Soft', deep: 'Deep' } as const)[cfg.surfaceDepth]} (shadow ${resolveDepth(cfg.surfaceDepth).elevation} — shadow ladder only) · Surface: ${({ outlined: 'Outlined (box + border)', filled: 'Filled (fill, no border)', plain: 'Plain (underline / seamless)' } as const)[cfg.surface]} · Border: ${cfg.borders} · Motion: ${'smooth'}
