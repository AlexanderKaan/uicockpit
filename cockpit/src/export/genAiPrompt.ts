...
- Radius: ${cfg.radius} — cards, inputs, dialogs AND buttons. One corner language; buttons never diverge.
...
- Elevation: ${({ flat: 'Flat', soft: 'Soft', deep: 'Deep' } as const)[cfg.surfaceDepth]} — how far cards/surfaces lift off the canvas via SHADOW only (shadow ${resolveDepth(cfg.surfaceDepth).elevation}). Flat = zero shadow (Linear/Vercel); Soft = subtle (shadcn/Stripe); Deep = strong shadow (Notion). Shadows auto-tint toward the brand hue. Surface separation (the grey ramp) is a SEPARATE axis — see Border/Surface below.
