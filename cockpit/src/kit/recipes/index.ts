...
.card {
  ...
  /* --k-card-gap completes the lazy hook set below: a scope that lays out its
     own internal rhythm (the configurator's control shell) needs to zero this,
     and doing it from outside meant out-specifying the recipe. */
  gap: var(--k-card-gap, var(--k-space, 16px));
...
.checkbox input[type="checkbox"] { border-radius: min(var(--k-radius-sm), 4px); }
...
.tab--on .tab__badge { background: var(--k-primary-soft); color: var(--k-primary-soft-fg); }`,
  },
  {
    ...
    section: "Table",
    css: `/* === Table === */
.table { width: 100%; border-collapse: collapse; font-size: var(--k-type-body); }
.table th, .table td {
...
.table thead { color: var(--k-fg-muted); font-weight: var(--k-weight-semibold); font-size: var(--k-type-eyebrow); text-transform: uppercase; letter-spacing: var(--k-track-eyebrow); }
.table tbody tr { border-top: var(--k-divider); }
.table tbody tr:hover { background: var(--k-state-hover); }
...
.table tbody tr:active { background: var(--k-state-press); }
...
.table th.is-sortable:hover { color: var(--k-fg); }
...
.is-sortable.is-active .table__sort-chevron { color: var(--k-primary-text); }
...
.table__name { font-weight: var(--k-weight-medium); color: var(--k-fg); }
.table__sub { font-size: var(--k-type-small); color: var(--k-fg-faint); margin-top: 1px; }
/* Numeric column (CP3 — the Attio/Linear "dense but calm" data signature). Value
   ...
   pro-data-table move every left-aligned table misses. On the .table ATOM (was
...
/* Tables are restrained shadcn-style: per-row border-top (base .table) + hover,
...
.table tr.table__group { border-top: var(--k-divider); }
.table tr.table__group:hover > * { background: var(--k-surface-sunken); }
...
.table--condensed { font-size: var(--k-type-small); }
.table--condensed th, .table--condensed td { padding: calc(var(--k-space, 10px) * 0.32) calc(var(--k-space, 10px) * 0.5); }
...
/* --- Card-framed table (.table--card) ---------------------------------------
   The bare .table uses border-collapse:collapse + per-row border-top — right for
   ...
   FF/WebKit). .table--card is the primitive for a STANDALONE sticky/rounded
...
.table--card { border-collapse: separate; border-spacing: 0; border: 1px solid var(--k-border); border-radius: var(--k-radius-lg); overflow: hidden; }
.table--card thead th { position: sticky; top: 0; z-index: 1; background: var(--k-surface); box-shadow: inset 0 -1px 0 var(--k-border); }
.table--card tbody tr { border-top: 0; }
.table--card tbody tr + tr td, .table--card tbody tr + tr th { box-shadow: inset 0 1px 0 var(--k-border); }
...
/* --- Hit-target floor (Invariant I4) --------------------------------------
   ...
   it isn't needed. */
.table tbody tr:has(input, button, a, [role="button"]) td { min-block-size: var(--k-hit-min); }

/* --- Frozen first column (.table__col--frozen) ------------------------------
   ...
   .table__col--frozen and wrap the table in an overflow-x:auto container. Needs
   .table--card (separate borders) so the sticky cell keeps its opaque background
   and dividers — collapsed borders vanish under a sticky cell. A one-sided
   shadow reveals that there's more to the right. */
.table__col--frozen { position: sticky; left: 0; z-index: 1; background: var(--k-surface); }
.table thead .table__col--frozen { z-index: 2; }
.table--card .table__col--frozen::after {
...
/* --- Responsive (Tailwind "hidden columns" + "stacked on mobile") --------
   Wrap a table in .table-responsive (a size container, so it adapts to its own
   width, not the viewport). Two levers:
     .table__col--optional  drop non-essential columns under a narrow host (mark
...
@container (max-width: 26rem) {
  ...
  .table-responsive .table--stack tbody td::before { content: attr(data-label); font-weight: var(--k-weight-semibold); color: var(--k-fg-muted); text-align: left; text-transform: none; letter-spacing: normal; }
  ...
  .table-responsive .table--stack .table__col--optional { display: none; }
}`,
  },
  {
    id: 'data-table',
    ...
    doc: {
      dos: [
        ...
        "On a narrow container let the container query reflow it; freeze the identity column with .table__col--frozen when it scrolls horizontally.",
      ],
      donts: [
    ...
    css: `/* === Data table ===
   The flagship BLOCK: a bordered frame that wraps the .table atom into a complete,
   believable data surface — matrix-complete across every state real data hits.
   It COMPOSES atoms (.toolbar · .table · .select / .select-trigger · .pagination),
...
@container datatable (max-width: 40rem) {
  ...
  .datatable--page .table { min-width: 34rem; }
  ...
  .datatable--page .table__col--frozen { box-shadow: 1px 0 0 var(--k-border); }
  ...
  .datatable--page .table thead .table__col--frozen { box-shadow: inset 0 -1px 0 var(--k-border), 1px 0 0 var(--k-border); }
}
...
.datatable .table td:first-child, .datatable .table th:first-child { padding-left: var(--k-s-12); }
.datatable .table td:last-child, .datatable .table th:last-child { padding-right: var(--k-s-12); }
...
/* Content-stress helper: .truncate (single-line ellipsis clamp) is now defined
   on the .table ATOM above, so every table — bare or inside .datatable — gets it,
...
.datatable__perpage .select { width: auto; min-height: var(--k-control-h-sm); }`,
  },
  {
    ...
    section: "Tooltip",
    css: `/* === Tooltip ===
   Hidden by default; reveals on hover/focus of the parent .tooltip wrapper.
   Add .tooltip--always for forced-open demos (gallery card uses this so users
...
.tooltip__pop--bottom { top: calc(100% + var(--k-s-6)); bottom: auto; }
.tooltip__pop--right { left: calc(100% + var(--k-s-6)); right: auto; top: 50%; bottom: auto; transform: translateY(-50%); }
.tooltip__pop--left { right: calc(100% + var(--k-s-6)); left: auto; top: 50%; bottom: auto; transform: translateY(-50%); }
.tooltip:hover .tooltip__pop,
.tooltip:focus-within .tooltip__pop,
...
.empty__grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(9rem, 100%), 1fr)); gap: var(--k-s-10); width: 100%; margin-top: var(--k-s-6); }`,
  },
  {
    id: 'select-trigger',
...
/* ---- Responsive (compact) ---- the .scaffold drops the nav to a BOTTOM area
 * below its 600px container width (M3 compact). There a vertical sidebar reads
 * wrong, so the sidebar REFLOWS to a horizontal, scrollable icon strip — a
 * mobile tab bar. Driven by the SCAFFOLD container (not the viewport), so it
 * also engages in the fixed-width loupe and in a consumer's real app. Brand,
 * group labels and the pinned footer collapse; each row keeps just its icon. */
@container scaffold (max-width: 600px) {
  .sidenav {
    flex-direction: row;
    align-items: center;
    width: 100%;
  ...
  .navrow__chev,
  .navsub { display: none; }
  .navrow { flex: 0 0 auto; justify-content: center; gap: 0; padding: var(--k-s-8) var(--k-s-10); position: relative; }
  .navrow .badge--count { position: absolute; top: var(--k-s-2); right: var(--k-s-4); min-width: var(--k-s-8); width: var(--k-s-8); height: var(--k-s-8); padding: 0; font-size: 0; border: var(--k-bw) solid var(--k-chrome-bg, var(--k-surface-sunken)); }
}`,
  },
  {
    id: 'appbar',
 ...
 * (reuses .in / .btn / .checkbox), a meta row (remember + forgot) and a footer
...
.calendar-week {
  ...
  border: 1px solid var(--k-border);
  border-radius: var(--k-radius-lg);
  /* ONE scroll container (the whole grid), so head + body share one scrollbar and
     their column dividers stay in register. overflow-y:auto also clips to the radius. */
  overflow-x: hidden; overflow-y: auto; max-height: 24rem; overscroll-behavior: contain;
  font-size: var(--k-type-caption);
}
.calendar-week--day { --cal-cols: 1; }
/* header - day name + date chip per column. STICKY: pins to the top on scroll. The
   bottom divider is an inset box-shadow (not border-bottom) so it travels with the
   sticky head and doesn't bleed under the scrolling grid. */
.calendar-week__head {
  display: grid; grid-template-columns: var(--cal-tpl);
  position: sticky; top: 0; z-index: 2;
  background: var(--k-surface);
  box-shadow: inset 0 -1px 0 var(--k-border);
}
...
.calendar-week__body { display: grid; grid-template-columns: var(--cal-tpl); }
...
/* current-time "now" line — the single most-used time-grid cue. Render ONE inside
   today's .calendar-week__col; --now = decimal hours after the grid's first hour
   (e.g. 9:30 on an 8AM grid = 1.5). A leading dot marks the time on the column
   edge. pointer-events:none so it never eats an event click; scrollIntoView it on
   mount so the grid opens on "now". */
.calendar-week__now {
  position: absolute; left: 0; right: 0;
  top: calc(var(--now, 0) * var(--k-cal-hour, 3rem));
  height: 0; border-top: 2px solid var(--k-accent);
  z-index: 2; pointer-events: none;
}
.calendar-week__now::before {
  content: ""; position: absolute; left: 0; top: 0;
  width: var(--k-s-8); height: var(--k-s-8); transform: translate(-50%, -50%);
  border-radius: var(--k-radius-pill); background: var(--k-accent);
}
/* an event - placed by --from (start hour) + --span (hours). Soft tonal fill +
   ...
   with --alt / --accent. OVERLAP: concurrent events split the column into lanes —
   the consumer sets --lanes (how many overlap) + --lane (this event's 0-based
   slot) and they sit side by side instead of hiding each other. Defaults
   (--lanes:1, --lane:0) = a single full-width event, the common case. */
...
.calendar-year { display: grid; grid-template-columns: repeat(auto-fill, minmax(min(12rem, 100%), 1fr)); gap: var(--k-s-20); }
...
/* ⚠️ minmax(min(12rem, 100%), 1fr), NOT minmax(12rem, 1fr). A minmax track does
   not shrink below its minimum, and rem SCALES WITH ZOOM — at 200% (WCAG 1.4.4)
   a 12rem floor becomes 384px, wider than the column holding it, and the footer
   hung 121px outside its card. min(…, 100%) lets the track collapse to whatever
   space actually exists.
   This was already the shape five other grids in this kit used. The right answer
   existed and was applied inconsistently, which is the shape of nearly every
...
.menu {
  ...
  /* AND IT STYLES THE RAIL IT JUST CREATED. A menu with a max-height scrolls by
     design, so leaving the scrollbar to the OS puts a heavy platform-grey rail
     down the side of a light overlay — which in dark mode is a black bar through
     the middle of a white panel.
     The kit already had the answer on .scroll-area and never applied it to the
     overlays that scroll. The evidence it had been noticed before: our own
     preview stylesheet carried a copy of this treatment, scoped to the preview
     and hard-coding hsl(0 0% 50% / .35) where a token belongs. Patching beside
     the recipe instead of in it is how a fix reaches one surface out of three. */
...
/* The TRAILING VALUE slot, and it exists because dogfooding asked for it.
   __shortcut is monospace on purpose — it holds a key hint. A settings menu puts
   the CURRENT VALUE in the same position, and those are proper nouns (Cobalt,
   Inter, Tonal) that read wrong in mono. Two different jobs sharing one slot is
   how a slot ends up serving neither, so the value gets its own.
   Found by rebuilding our own configurator panel out of this recipe: the kit had
   every other part of that row already. */
.menu::-webkit-scrollbar { width: var(--k-s-6); height: var(--k-s-6); }
.menu::-webkit-scrollbar-track { background: transparent; }
.menu::-webkit-scrollbar-thumb { background: var(--k-border); border-radius: var(--k-radius-pill, 999px); }
.menu::-webkit-scrollbar-thumb:hover { background: var(--k-fg-faint); }
.menu__value { margin-left: auto; font-size: var(--k-type-small); color: var(--k-fg-muted); font-weight: var(--k-ui-weight, 500); }
 ...
 *
 * This recipe's APG anchor claims the Spinbutton pattern and lists Up/Down,
 * Page Up/Page Down and Home/End. For a long time the markup was an <input> with
 * NO type at all — a text field — so not one of those keys did anything. The kit
 * documented a behaviour contract it did not keep, and the doc page said so in
 * print on a public URL.
 *
 * The fix is not to implement the pattern in JavaScript. type="number" IS the
 * spinbutton: the browser gives Up/Down, the numeric keyboard, min/max clamping
 * and the range announced to assistive tech, for free and better than we would.
 * We supply only what the platform does not: the visual stepper.
 *
 * (Which is the opposite call from .memorable-date, deliberately. GOV.UK's
 * research is that type="number" is WRONG for a date or an account number —
 * scroll-wheel mutation, leading zeros eaten, no maxlength — so that recipe uses
 * inputmode="numeric" on a text field. A quantity is the case type="number" was
 * designed for. Same question, opposite answers, both from the same source.)
 *
 * The browser's own spinner is suppressed, because we draw the stepper. */
.numinput { padding: 0; }
.numinput__field { font-variant-numeric: tabular-nums; text-align: center; }
.numinput__field::-webkit-outer-spin-button,
.numinput__field::-webkit-inner-spin-button { appearance: none; margin: 0; }
.numinput__field[type='number'] { appearance: textfield; -moz-appearance: textfield; }
.numinput__step {
...
.numinput__step:hover:not(:disabled) { background: var(--k-state-hover); color: var(--k-fg); }
/* PRESS. The old rule recoloured the glyph and nothing else, which on a filled
   field is close to invisible — the reason the control felt unresponsive. A
   press now moves the surface, which is what every other control in the kit
   does. */
.numinput__step:active:not(:disabled) { background: var(--k-state-press); color: var(--k-primary-text); }
/* AT THE BOUND, SAY SO. A button that stays lit and does nothing is worse than
   no button: the person presses it, nothing happens, and there is no way to know
   whether the control is broken or the value is already at its limit. */
.numinput__step:disabled { color: var(--k-fg-faint); cursor: not-allowed; background: transparent; }
...
.numinput__chev:hover:not(:disabled) { background: var(--k-state-hover); color: var(--k-fg); }
.numinput__chev:active:not(:disabled) { background: var(--k-state-press); color: var(--k-primary-text); }
.numinput__chev:disabled { color: var(--k-fg-faint); cursor: not-allowed; background: transparent; }
.numinput__chev + .numinput__chev { border-top: var(--k-divider); }
/* ⚠️ THESE FOUR SELECTORS USED TO SET outline: none, and the comment justifying
 * it said the wrapper's :focus-within ring "covers keyboard users". It does not.
 * The wrapper holds THREE tab stops — minus, field, plus — and the wrapper ring
 * looks identical for all three, so a keyboard user tabbing through had no way
 * to tell which control they were on. That is 2.4.7 failing while a comment
 * claims it passes, which is the worst kind of both.
 *
 * The buttons carry their own indicator, drawn INSIDE so it cannot be clipped by
 * the wrapper and does not fight the wrapper's own ring. */
.numinput__chev:focus-visible,
.numinput__step:focus-visible {
  outline: none;
  box-shadow: inset 0 0 0 var(--k-ring-w, 2px) var(--k-ring);
  color: var(--k-fg);
}`,
  },
  {
...
/* Hit-target floor (Invariant I4) — an interactive row clears the WCAG-2.2 24px
   target even at Compact density; generalises the .table guard so list rows stop
...
.slotpicker { display: grid; grid-template-columns: repeat(auto-fill, minmax(min(82px, 100%), 1fr)); gap: var(--k-stack-gap, 8px); }
.slot {
