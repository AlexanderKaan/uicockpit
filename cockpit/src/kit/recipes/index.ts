...
/* Scroll wrapper — on a narrow container the matrix scrolls horizontally
   instead of squishing its columns below readability (the same convention as
   the data table). Wrap the grid in this on any width-constrained surface. */
.plan-compare-scroll { overflow-x: auto; }
.plan-compare {
  display: grid;
  /* Keeps the columns readable; below this the wrapper scrolls. */
  min-width: 30rem;
