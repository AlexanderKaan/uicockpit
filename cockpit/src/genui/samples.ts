...
/**
 * ONE sample per catalogue type — the smallest node that shows the type's
 * fields. Two consumers, so the two cannot drift: the sandbox's "What you can
 * ask for" reference renders and INSERTS these, and genui.test.ts admits and
 * renders every one in kit classes. A type without a sample fails the test; a
 * sample that does not admit fails the test — so what the reference offers is
 * exactly what the renderer can paint.
 */
export const SAMPLES: Record<GenType, GenNode> = {
  ...
  link: { type: 'link', text: 'Read the full decision', href: '#' },
  ...
  card: { type: 'card', title: 'A card', desc: 'With a description.', badge: { text: 'New', tone: 'primary' }, media: { alt: 'A media slot', label: 'Media' }, children: [{ type: 'text', text: 'Content in the body.' }], actions: [{ type: 'button', text: 'Primary', variant: 'primary', size: 'sm' }, { type: 'button', text: 'Later', variant: 'ghost', size: 'sm' }] },
  button: { type: 'button', text: 'Continue', variant: 'primary', icon: 'chevR' },
  badge: { type: 'badge', text: 'Under review', tone: 'info' },
  ...
  banner: { type: 'banner', strong: 'Maintenance', text: 'Mijn Omgeving is unavailable on Sunday 02:00–04:00.', link: { text: 'More', href: '#' } },
  ...
  progress: { type: 'progress', label: 'Storage', value: 7.8, max: 10, unit: 'GB', hint: '2.2 GB left this month' },
