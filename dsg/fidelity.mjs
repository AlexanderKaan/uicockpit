...
for (const id of IDS) {
  const sheet = css[id] ?? ''
  const used = classesOf(markup(id))
  /* A class exists if the kit's own compiled CSS names it as a selector.
   * Tailwind BACKSLASH-ESCAPES every character a CSS identifier may not carry,
   * so `hover:bg-primary/90` is written `.hover\:bg-primary\/90:hover`. The
   * first version of this meter looked for the literal name and reported nine
   * of shadcn's own strings as invented — a meter that cannot read the sheet
   * accuses the kit. Each non-identifier character may be preceded by one. */
  const sel = (c) => [...c].map((ch) => (/[A-Za-z0-9_-]/.test(ch) ? ch
    : `\\\\?${ch.replace(/[.*+?^${}()|[\]\\/]/g, '\\$&')}`)).join('')
  const missing = used.filter((c) => !new RegExp(`\\.${sel(c)}(?=[\\s,{:.>~+\\[)])`).test(sheet))
  const pct = used.length ? Math.round(((used.length - missing.length) / used.length) * 100) : 100
  ...
  const inline = (markup(id).match(/style="/g) ?? []).length
  if (!used.length) {
  ...
  console.log(`  ${kits[id].name.padEnd(14)} ${String(pct).padStart(3)}%   ${used.length - missing.length}/${used.length} classes are theirs${note}`)
